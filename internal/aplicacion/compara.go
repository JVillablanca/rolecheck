...
func ComparaCuentas() {
	...
	fmt.Fprintf(f, "Roles en Cuenta 1 y no en Cuenta 2:\n")
	for r := range roles1 {
		if !roles2[r] {
			fmt.Fprintf(f, "- %s\n", r)
		}
	}
	fmt.Fprintf(f, "\nRoles en Cuenta 2 y no en Cuenta 1:\n")
	for r := range roles2 {
		if !roles1[r] {
			fmt.Fprintf(f, "- %s\n", r)
		}
	...
	fmt.Fprintf(f, "Permisos en Cuenta 1 y no en Cuenta 2:\n")
	for k, p := range perm1 {
		if _, ok := perm2[k]; !ok {
			fmt.Fprintf(f, "- %+v\n", p)
		}
	}
	fmt.Fprintf(f, "\nPermisos en Cuenta 2 y no en Cuenta 1:\n")
	for k, p := range perm2 {
		if _, ok := perm1[k]; !ok {
			fmt.Fprintf(f, "- %+v\n", p)
		}
	...
	fmt.Fprintf(f, "Data Access en Cuenta 1 y no en Cuenta 2:\n")
	for k, d := range data1 {
		if _, ok := data2[k]; !ok {
			fmt.Fprintf(f, "- %+v\n", d)
		}
	}
	fmt.Fprintf(f, "\nData Access en Cuenta 2 y no en Cuenta 1:\n")
	for k, d := range data2 {
		if _, ok := data1[k]; !ok {
			fmt.Fprintf(f, "- %+v\n", d)
		}
